...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    ...
    merge_article = Article.find_by_id(params[:merge_id])
    if merge_article.blank? 
      redirect_to :action => 'index'
      flash[:error] = "Article ID '#{params[:merge_id]}' does not exist."
      return
    end
    article.merge_with(merge_article)
    redirect_to :action => 'index'
    flash[:notice] = "Article ID '#{params[:article_id]}' and '#{params[:merge_id]}' merged"
  end
